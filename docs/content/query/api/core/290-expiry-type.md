...
`Expiry` sets the three timing tiers, all in milliseconds. Memory and local are different **tiers**, not different degrees of strictness: memory is a small RAM cache of queries currently or recently observed — seconds to minutes. Local is the durable superset on disk — days to weeks. 100 MB is fine in local; it is not fine in memory.
...
- `local` — how long a query is retained in the local store after its last refresh. Default: `2_592_000_000` (30 days). The local purge uses this threshold and runs at most once per `local / 8`.

All timing settings take effect only when [tick](api.html#tick) runs — the engine owns no timers.
