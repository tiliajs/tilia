...
- `pending` — number of ops waiting in the outbox. Every [upsert](api.html#upsert) and [remove](api.html#remove) counts immediately, including offline writes: `remote.push` is never called while offline, so the count only drains once online.
