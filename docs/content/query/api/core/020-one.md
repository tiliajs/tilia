...
tags: []
signature.ts: "one: (query: Q) => Loadable<T>"
signature.res: "one: 'query => loadable<'a>"
label: one
---

`one` reads the first result of a query, as ordered by the `sort` given to [make](api.html#make).

The read is reactive: inside `observe`, `watch`, or a component, the caller reruns when the result changes. Reading the same query from many places shares one entry — it does not multiply fetches.
