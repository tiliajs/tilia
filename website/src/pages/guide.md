...
title: Tilia Documentation - Complete Guide
description: Complete guide to understanding and using Tilia, a simple and performant state management library. Documentation with practical examples.
keywords: tilia documentation, complete guide, state management, reactive programming, FRP, domain-driven design, React, TypeScript, ReScript
---

<main class="container mx-auto px-6 py-8 max-w-4xl">
<section class="header">

# Tilia Documentation {.documentation}

Complete guide to understanding and using Tilia, a simple and performant state management library. {.subtitle}
...
The `tilia()` function transforms an ordinary JavaScript object into a reactive object:
...
user.age = 26;                  // Normal modification
user.preferences.theme = "light"; // Nested objects are also reactive
```
...
// Use it like a normal object
Js.log(user.name)         // "Alice"
user.age = 26                  // Normal modification
user.preferences.theme = "light" // Nested objects are also reactive
```

**Key points:**
- The returned object behaves exactly like a normal object
- All nested objects are automatically made reactive
- Arrays are also supported

```typescript
const todos = tilia({
  items: [
    { id: 1, text: "Learn Tilia", done: false },
    { id: 2, text: "Create an app", done: false },
  ],
});
...
// Array operations are tracked
todos.items = Array.concat(todos.items, [{id: 3, text: "Deploy", done: false}])
todos.items[0].done = true
```
...
let (counter, setCounter) = signal(0)

observe(() => {
  Js.log2("Counter:", Int.toString(counter.value))
})
// Immediate output: "Counter: 0"

setCounter(1)  // Output: "Counter: 1"
setCounter(2)  // Output: "Counter: 2"
```

**⚠️ Important note:** If you mutate an observed value in the `observe` callback, it will be re-executed after it ends. This allows implementing state machines.

```typescript
observe(() => {
  console.log("Value:", state.value);
  if (state.value < 10) {
    state.value++;  // ⚠️ Causes a re-execution
  }
});
```
...
observe(() => {
  Js.log2("Value:", Int.toString(state.value))
  if state.value < 10 {
    state.value = state.value + 1  // ⚠️ Causes a re-execution
  }
})
```
...
watch(
  // Capture function: defines dependencies
  () => exercise.result,
  
  // Effect function: called when dependencies change
  (result) => {
    if (result === "pass") {
      alice.score++;  // This modification is NOT observed
    } else if (result === "fail") {
      alice.score--;
    }
  }
...
exercise.result = "pass";  // ✨ Triggers the effect
alice.score = 100;         // 😴 Does NOT trigger the effect
```

```rescript
open Tilia

let exercise = tilia({result: "pending"})
let (aliceScore, setAliceScore) = signal(0)
...
watch(
  ...
  (result) => {
    if result == "pass" {
      setAliceScore(aliceScore.value + 1)  // This modification is NOT observed
    } else if result == "fail" {
      setAliceScore(aliceScore.value - 1)
    }
  }
)

exercise.result = "pass"  // ✨ Triggers the effect
setAliceScore(100)         // 😴 Does NOT trigger the effect
```

**Key difference with `observe()`:**
- In `watch`, modifications in the effect do not trigger re-execution
- Useful for avoiding infinite loops in complex cases
...
**💡 Pro tip** `batch` is not required in `computed`, `source`, `store`, `observe` or `watch` where notifications are already blocked. {.pro}
...
const alice = tilia({
  ...
  // The 'age' value is always up-to-date
...
let _ = Js.Global.setInterval(() => globals.now = dayjs(), 1000 * 60)
...
**💡 Pro tip:** The computed can be created anywhere but only becomes active once inserted in a Tilia object. {.pro}

Once a value is computed, it behaves exactly like a regular value until it expires due to a change in dependencies. This means there is nearly zero overhead for computed values acting as getters.
...
Before introducing each one, here's an overview. {.subtitle}
...
A source is similar to a computed, but it receives an initial value and a setter function and does not return a value. The setup callback is called on first value read and whenever any observed value changes. The initial value is used before the first set call.
...
let app = tilia({
  // Async data (re-)loader (setup will re-run when alice.age changes)
  social: source(
    #Loading,
    (_previous, set) => {
      if alice.age > 13 {
...
- Receives the previous value as the first argument of the callback
- The callback is re-executed when its dependencies change
- Ideal for reactive data loaders
...
let loggedOut = (set: Setter.t<auth>): auth => {
  #LoggedOut({
    login: (user: user) => set(loggedIn(set, user)),
  })
}

let loggedIn = (set: Setter.t<auth>, user: user): auth => {
  #LoggedIn({
    user: user,
    logout: () => set(loggedOut(set)),
  })
...
```rescript
let derived = (fn: unit => 'a): signal<'a> => {
  signal(computed(fn))
}
...
let list = (todos: todos) => {
  let compare = todos.sort == "by date"
    ? (a, b) => String.compare(a.createdAt, b.createdAt)
    : (a, b) => String.compare(a.title, b.title)
  Array.sortBy(todos.data, compare)
}

// A pure function for toggling a todo, also easily testable.
let toggle = ({data, repo}: todos) => {
  (id: string) => {
    switch Array.getBy(data, (t) => t.id == id) {
    | Some(todo) =>
      todo.completed = !todo.completed
      repo->save(todo)
    | None => raise(Failure(`Todo ${id} not found`))
    }
  }
}

// Injecting the dependency "repo"
let makeTodos = (repo: repo) => {
  // ✨ Carve the todos feature ✨
  carve(({derived}) => {
    sort: "by date",
    list: derived(list),
...
Insert `useTilia` at the top of React components that consume tilia values.
...
With this helper, TodoView does not depend on `app.todos.selected.id` but on `selected.value`. This prevents the component from re-rendering on every change to the selected todo.
